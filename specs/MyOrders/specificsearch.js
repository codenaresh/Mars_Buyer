      ...
      getPaginationsize.count().then(function (pagination) {
          ...
          if (pagination > 0) {
              ...
              for (var i = 1; i < pagination-1; i++) {
      			...
      			element.all(by.css(".table>thead")).getText().then(function(text){
      				
      				
      				console.log("text val is :"+text[0]);
      				
      			});
      			
      		
                  
      			tbledata.all(by.tagName("tr")).count().then(function(rowcount){
      				
      				console.log("row counts is :"+rowcount);
